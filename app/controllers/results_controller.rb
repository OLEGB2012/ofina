    ...
    unless @f1_beg.nil? and @f1_end.nil? 
      ...
      when 1
        ...
        unless @form_one_reports.empty?
          # Готовим целевые таблицы - чистим за период по предприятию определённый тип диаграмм... 
          # (записи из balanse_values "уйдут" по каскаду)...
          BalanseRow.BalanseRowEnterpriseFor(params[:id]).WorkPeriod(@f1_beg.date_period,@f1_end.date_period).DiagramType(1).destroy_all
          # ... и заполняем ...
          # ################################################################
          @Arr_1=[["1", "Раздел I. ДОЛГОСРОЧНЫЕ АКТИВЫ", "S190"],
      ...
      when 3
        ...
        #
           unless @form_one_reports.empty?
              ...
              # ... и заполняем ...
              # ################################################################
              @Arr_1=[["1", "Раздел II. КРАТКОСРОЧНЫЕ АКТИВЫ", "S290"],
                    ["1", "Запасы", "S210"],
      ...
      when 4
        ...
        #
          unless @form_one_reports.empty?
              ...
              # ... и заполняем ...
              # ################################################################
              @Arr_1=[["1", "Раздел III. СОБСТВЕННЫЙ КАПИТАЛ", "S490"],
      ...
      @Arr_1.each do |x|       
        eval("@BR_new_rec=BalanseRow.create!(date_period_beg: @f1_beg.date_period, 
                ...
                @DiagType#{x[0]}_data  =BalanseRow.BalanseRowEnterpriseFor(params[:id]).DiagramType(#{x[0]}).WorkPeriod(@f1_beg.date_period,@f1_end.date_period).all
